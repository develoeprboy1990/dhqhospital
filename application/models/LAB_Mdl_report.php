...
class LAB_Mdl_report extends CI_Model
  {
    ...
    $sql = "SELECT date AS date, SUM(CASE WHEN UPPER(gander) = 'Male' THEN 1 ELSE 0 END) AS Male,
      ...
      public function all_lab_report($search,$from,$to,$gendar,$shift,$type,$search_dept_report)
      {
        ...
        date_add($to2,date_interval_create_from_date_string("+32 HOUR"));
        $to2 = date_format($to2,"Y-m-d H:i:s");
        $sql = "SELECT * FROM `lab_entry` WHERE `id`!='' AND dept_id='16' AND is_deleted=0 AND date>='$from2' AND date<'$to2'";
...
public function today_lab_report($dept_id){
        ...
        $to = date_format($to,"Y-m-d H:i:s");
        $year = date('y');
        $sql = "SELECT * FROM `lab_entry` WHERE `id`!='' AND dept_id='$dept_id' AND yearly_no='$year' ".$userType." AND is_deleted=0 AND date>='$from' AND date<'$to' ORDER BY id DESC LIMIT 60"; 
        $query=$this->db->query($sql);
        return $query;    
} 
...
(SELECT COUNT(*) FROM lab_entry WHERE 1 $dept_id $date2 $shift AND type = 'SSP') as SSPReceipts,
(SELECT SUM(tests) FROM lab_entry WHERE 1 $dept_id $date2 $shift AND type = 'SSP') as SSPTests,
(SELECT SUM(price) FROM lab_entry WHERE 1 $dept_id $date2 $shift AND type = 'SSP') as SSPTestsAmount
...
public function lab_print_report_delete($receptNumber,$yearly_no)
  ...
  public function test_wise_report($search,$from,$to,$test_id,$dept_id)
  {
      ...
      if(isset($test_id) && !empty($test_id)){
        ...
        $sql = "SELECT test_id,  
                SUM(CASE WHEN types='Ward'  THEN 1 ELSE 0 END) AS WardTest,
                SUM(CASE WHEN types='Casualty'  THEN 1 ELSE 0 END) AS CasualtyTest,
                SUM(CASE WHEN types='Labour_Room'  THEN 1 ELSE 0 END) AS LRoomTest,
                SUM(CASE WHEN types='SSP'  THEN 1 ELSE 0 END) AS SSPTest,
                SUM(CASE WHEN types='Entitled'  THEN 1 ELSE 0 END) AS EntitledTest,
                SUM(CASE WHEN types='Paid'  THEN 1 ELSE 0 END) AS PaidTest,
                COUNT(test_id) AS TotalTests
FROM lab_entry_details WHERE `id`!='' AND `is_deleted`=0 AND `date` >= '$from' and `date` <'$to' $test_sql $dept  GROUP By test_id";
