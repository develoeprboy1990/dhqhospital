    ...
    <a style="float: right; margin: 10px;" href="<?php echo base_url('Product/add_type');?>"><button class="btn btn-primary">Add New</button> </a>
    <div class="table table-striped table-bordered table-responsive" style="width: 100%; background-color: #e4e0e040; margin-top: 10px;">
        <table id="example1" class="table table-hover table-bordered" style="background-color: #ebebe0;">
            ...
            <?php
                ...
                $this->db->select('*');
                $this->db->where('id',$row->indent_id);
                $result = $this->db->get('departments')->result_array();
            ?>
                <tr>
                    <td><?php echo $i; ?></td>
                    <td><?php echo $row->name; ?></td>
                    <td><?php echo $result[0]['dep_name']; ?></td>
                    <td>
                    <a class="btn-sm btn-success" href="<?php echo base_url("/Product/edit_type/".$row->id);?>"><i class="fa fa-pencil"></i></a>
    ...
    function validate(a)
    {
        ...
        swal({
                ...
                closeOnConfirm: false }, function()
            {
                ...
                $(location).attr('href','<?php echo base_url()?>/Product/delete_type/'+id);
            }
        );
    }
    </script>