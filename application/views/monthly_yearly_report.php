            ...
            <table class="table table-bordered" style="border:1px solid #F2DEDE;"> 
              ...
              <tbody>
                ...
                if(count($dept) > 0){  
                  ...
                  $junuary_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$January[from_date]' AND date<'$January[to_date]' AND dept_id=".$row->id;
                 $junuary_query = $this->db->query($junuary_sql)->result_array();
                 
                 $feb_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$February[from_date]' AND date<'$February[to_date]' AND dept_id=".$row->id;
                 $feb_query = $this->db->query($feb_sql)->result_array();

                 $march_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$March[from_date]' AND date<'$March[to_date]' AND dept_id=".$row->id;
                 $march_query = $this->db->query($march_sql)->result_array();

                 $april_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$April[from_date]' AND date<'$April[to_date]' AND dept_id=".$row->id;
                 $april_query = $this->db->query($april_sql)->result_array();

                 $may_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$May[from_date]' AND date<'$May[to_date]' AND dept_id=".$row->id;
                 $may_query = $this->db->query($may_sql)->result_array();

                 $june_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$June[from_date]' AND date<'$June[to_date]' AND dept_id=".$row->id;
                 $june_query = $this->db->query($june_sql)->result_array();

                 $july_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$July[from_date]' AND date<'$July[to_date]' AND dept_id=".$row->id;
                 $july_query = $this->db->query($july_sql)->result_array();

                 $august_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$August[from_date]' AND date<'$August[to_date]' AND dept_id=".$row->id;
                 $august_query = $this->db->query($august_sql)->result_array();

                 $sep_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$September[from_date]' AND date<'$September[to_date]' AND dept_id=".$row->id;
                 $sep_query = $this->db->query($sep_sql)->result_array();

                 $oct_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$October[from_date]' AND date<'$October[to_date]' AND dept_id=".$row->id;
                 $oct_query = $this->db->query($oct_sql)->result_array();

                 $nov_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$November[from_date]' AND date<'$November[to_date]' AND dept_id=".$row->id;
                 $nov_query = $this->db->query($nov_sql)->result_array();

                 $dec_sql = "SELECT ".$type_sql." FROM ".$table." WHERE `is_deleted`=0 AND yearly_no='$yearly_no' AND date>='$December[from_date]' AND date<'$December[to_date]' AND dept_id=".$row->id;
