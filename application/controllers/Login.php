...

{
...

		{
...

			{
...

				{
					...
					if($data->shift != $this->input->post('shift'))

					{

						$this->session->set_flashdata('msg','<div class="text-danger">You are '.$data->shift.' shift user. can not login in '.$c_shift.'!!!</div>');
					...
					{ 	
						...
						$this->session->set_flashdata('msg','<div class="text-danger">You are '.$data->shift.' user. can not login in '.$c_shift.'!!!</div>');
...

                {
					...
					elseif($data->security_type== '0') {
		                ...
		                $this->session->set_userdata('full_name',$name.' '.$l_name);
						...
						//print_r($this->session->userdata('dept_ids')); die();
...

	{
		...
		if($this->session->flashdata('e_msg') != null)

		{

			$this->session->set_flashdata('msg','<div class="text-danger">'.$this->session->flashdata('e_msg').'</div>');
