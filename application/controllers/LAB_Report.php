...
public function dailly_summary_report()
	...
	public function waqar45404()
	{
		$data['getDept']=$this->LAB_Mdl_report->getDept();
		$data['page_name1'] = 'my_dailly_LAB_summary_report';
		$data['page_name'] = 'my_dailly_LAB_summary_report';
		$this->load->view('index',$data);
	}
	public function all_report()
	{
		...
		$data['getDept']=$this->LAB_Mdl_report->getDept();
		$data['query'] = $this->LAB_Mdl_report->all_lab_report($search,$from,$to,$gendar,$shift,$type,$dept_id);
...
foreach($query_lab_test->result() as $row_lab_test){
	...
	public function test_wise_report()
	{
		...
		//$data['DayAmount'] = $this->LAB_Mdl_report->test_wise_day_amount($from,$to);
		$data['fromDate'] = $from;
		$data['toDate'] = $to;
	...
	public function PaidQuery($shift, $date){
	  return $this->db->query("SELECT SUM(price) AS ".$shift."Paid FROM lab_entry WHERE yearly_no='22' AND is_deleted=0 AND dept_id='16' ".$date." AND type='Paid' AND shift = '".$shift."'")->row();
	}
}
