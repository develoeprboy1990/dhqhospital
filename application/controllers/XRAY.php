...
class XRAY extends CI_Controller {
	...
	public function add()
	{
		...
		if(check_shift($this->input->post('shift'))){
		...
		echo 'success';
			...
			$this->session->set_flashdata('e_msg','<div class="text-danger">You are '.$this->input->post('shift').' user. can not entry in '.get_current_shift().'!!!</div>');
		echo 'fail';
		}
	}
